...
use datafusion_physical_plan::aggregates::{AggregateMode, LimitConfig};
...
impl protobuf::PhysicalPlanNode {
    ...
    fn try_into_aggregate_physical_plan(
        ...
        let agg = if let Some(limit_proto) = &hash_agg.limit {
            ...
            let limit_config = match limit_proto.descending {
                Some(descending) => LimitConfig::new_with_order(limit, descending),
                None => LimitConfig::new(limit),
            };
            agg.with_limit_config(Some(limit_config))
        } else {
            agg
        };
    ...
    fn try_from_aggregate_exec(
        ...
        let limit = exec.limit_config().map(|config| protobuf::AggLimit {
