...
impl Default for WindowTopK {
    fn default() -> Self {
        Self::new()
    }
}

impl PhysicalOptimizerRule for WindowTopK {
    fn optimize(
        &self,
...
fn try_optimize(plan: &Arc<dyn ExecutionPlan>) -> Option<Arc<dyn ExecutionPlan>> {
    ...
    // Keep Arc references to avoid cloning inner plan nodes during reconstruction.
    let filter_input = filter.input();
    let (window, projection, window_arc) =
        match filter_input.as_any().downcast_ref::<ProjectionExec>() {
            Some(proj) => {
                ...
                (w, Some(proj), Arc::clone(proj.input()))
            }
            None => {
                let w = filter_input
                    .as_any()
                    .downcast_ref::<BoundedWindowAggExec>()?;
                (w, None, Arc::clone(filter_input))
            }
        };
    ...
    if !partition_exprs_match(&sort.expr()[..partition_len], partition_by) {
        return None;
    }
    ...
    // Reconstruct the plan tree using with_new_children on existing Arcs
    // to avoid expensive clones of inner plan nodes
    let new_window = window_arc.with_new_children(vec![partitioned_sort]).ok()?;
    let new_filter_input = if projection.is_some() {
...
fn find_row_number_limit(
    ...
    for conjunct in &conjuncts {
        let Some(topk_info) = extract_topk_from_binary(conjunct) else {
            continue;
        };

        let Some(window_expr_idx) =
            resolve_column_to_window_expr_index(&topk_info.col, window, projection)
        else {
            continue;
        };
        ...
        if is_row_number_expr(&window_exprs[window_expr_idx]) {
...
fn extract_topk_from_binary(expr: &Arc<dyn PhysicalExpr>) -> Option<TopKInfo> {
    ...
    let (col, limit_value, operator) = if let Some(col) =
    ...
    let adjusted_limit = match operator {
        Operator::Lt => limit.checked_sub(1)?,
        Operator::LtEq | Operator::Eq => limit,
        _ => return None,
    };
